...
main.addEventListener('drop', async (ev) => {
    ...
    if (fileList.length != 1) {
        ...
        let xhr = new XMLHttpRequest();
        let url = 'http://192.168.1.99:80/upload';
        let urlStatus = 'http://192.168.1.99:80/status'
...
trimBtn.addEventListener('click', async () => {
    let xhr = new XMLHttpRequest();
    let url = 'http://192.168.1.99:80/trim';
...
downloadBtn.addEventListener('click', async (e) => {
    try {
        ...
        const url = `http://192.168.1.99:80/download/${filename}`;
